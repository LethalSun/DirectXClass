...
void LoadHeightMap()
{
	vector<BYTE> in(numVertices);
	ifstream loadFile;
	loadFile.open(L"heightMap.raw", ios_base::binary);

	if (loadFile)
	{
		loadFile.read((char*)&in[0], (streamsize)in.size());
		loadFile.close();
	}
...
void CalCulateMatrixForHeightMap(float dt)
{
	XMMATRIX mat = XMMatrixRotationY(0.f);
...
void CreateHeightMapIB()
{
	...
	for (int z = 0; z < _numVertsPerRow - 1; ++z)
	{
		...
		{
			indices[baseindex] = z*_numVertsPerRow + x;
			indices[baseindex+2] = z*_numVertsPerRow + x+1;
			indices[baseindex+1] = (z+1)*_numVertsPerRow + x;

			indices[baseindex+3] = (z+1)*_numVertsPerRow + x;
			indices[baseindex + 5] = z*_numVertsPerRow + x + 1;
...
void CreateHightMapVB()
{
	...
	bd.ByteWidth = sizeof(MyVertex)*numVertices;// 버퍼 크기
	...
	//숙제는 이부분을 cpu가 접근하도 록 허용한후 heightmapVertex 를 실시간으로 수정해야 한다.
	//D3D11_USAGE_DEFAULT->D3D11_USAGE_DYNAMIC
	D3D11_SUBRESOURCE_DATA initData;
	ZeroMemory(&initData, sizeof(initData));
	initData.pSysMem = heightmapVertex;// 초기화하기 위한 버퍼 배열 포인터
	g_pd3dDevice->CreateBuffer(&bd,// 생성할 버퍼의 정보를 담은 구조체
		&initData, // 버퍼 초기화시 필요한 데이터
		&g_pHeightMapVertexBuffer);// 생성된 버퍼
}
...
void InitMatrix()
{
	...
	//View Matrix
	XMVECTOR pos = XMVectorSet(-104.0f, 506.0f, 127.0f, 1.0f);
	XMVECTOR target = XMVectorSet(0.53f, -0.83f, 0.0f, 0.092f);
	XMVECTOR up = XMVectorSet(0.82f, 0.54f, 0.14f, 0.0f);
